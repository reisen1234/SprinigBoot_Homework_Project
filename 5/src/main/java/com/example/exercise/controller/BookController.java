...
public class BookController {
    ...
    public Object saveBook(@RequestBody BookDto bookDto){
        ...
        LambdaQueryWrapper<User> userLambdaQueryWrapper = new LambdaQueryWrapper<User>().eq(User::getId,bookDto.getUserId());
    ...
    public Object pageByUsername(@ApiParam("Page`s Index") @RequestParam("pageNum") Integer pageNum,
                                 @ApiParam("Page`s size") @RequestParam("pageSize") Integer pageSize,
                                 @ApiParam("Book username") @RequestParam("username") String username) {
        LambdaQueryWrapper<User> userLambdaQueryWrapper = new LambdaQueryWrapper<User>().eq(User::getName, username);
        User user = userService.getOne(userLambdaQueryWrapper);
        if(user == null)
            return new Result<>(ResultType.FAIL.getCode(), "没有这名用户",null);
        UserDto userDto = BeanUtil.copyProperties(user,UserDto.class);
        Page<BookDto> bookDtoPage = bookService.pageByUsername(pageNum,pageSize,user.getId(),userDto);
